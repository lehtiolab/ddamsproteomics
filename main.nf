...
process msgfPlus {
  ...
  ${params.mods.contains('Unknown') ? "sed -i '/unknown modification/s/PSI-MS/UNIMOD/' '${sample}.mzid'" : ''}
  """
}
...
process mergePTMPeps {
  ...
  ${!params.onlypeptides ? "head -n1 mergedtable | sed \$'s/Peptide sequence/Peptide sequence\tMaster protein(s)\tGene name(s)\tPTM flanking sequence(s)\tMatching gene count/'> '${peptable}'" : ''}
